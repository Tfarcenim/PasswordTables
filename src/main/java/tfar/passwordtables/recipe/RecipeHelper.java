...
public class RecipeHelper {
	...
	public boolean canCraft(Recipe recipe, @Nullable IntList packedItemList) {
		return this.canCraft(recipe, packedItemList, 1);
	}

	public boolean canCraft(Recipe recipe, @Nullable IntList packedItemList, int maxAmount) {
	...
	class RecipePicker {
		...
		public RecipePicker(Recipe recipeIn) {
			this.recipe = recipeIn;
			this.ingredients.addAll(recipeIn.getInputs());
		...
		public boolean tryPick(int maxAmount, @Nullable IntList listIn) {
			if (maxAmount <= 0) {
				...
				for (k = 0; this.dfs(maxAmount); ++k) {
					RecipeHelper.this.tryTake(this.possessedIngredientStacks[this.path.getInt(0)], maxAmount);
					...
					if (flag1 && ingredient == Ingredient.EMPTY) {
						...
						for (int l1 = 0; l1 < this.possessedIngredientStackCount; ++l1) {
							if (this.hasResidual(false, j1, l1)) {
								...
								RecipeHelper.this.increment(this.possessedIngredientStacks[l1], maxAmount);
		...
		private int[] getUniqueAvailIngredientItems() {
			...
			while (intiterator.hasNext()) {
				if (!RecipeHelper.this.containsItem(intiterator.nextInt())) {
					intiterator.remove();
				}
			}
		...
		private boolean dfs(int amount) {
			...
			for (int l = 0; l < k; ++l) {
				if (RecipeHelper.this.itemToCount.get(this.possessedIngredientStacks[l]) >= amount) {
					this.visit(false, l);
		...
		private int getMinIngredientCount() {
			...
			for (Ingredient ingredient : this.ingredients) {
				...
				for (IntListIterator intlistiterator = ingredient.getValidItemStacksPacked().iterator(); intlistiterator.hasNext(); l = Math.max(l, RecipeHelper.this.itemToCount.get(i1))) {
